...
const Auth = (() => {
  ...
  function init() {
    // Attach main sidebar button immediately so it never breaks
  ...
  function updateUI() {
    ...
    if (btnAuth) {
      if (currentUser) {
        btnAuth.innerHTML = `<img src="${currentUser.photoURL}" style="width:24px; height:24px; border-radius:50%;"> <span>Logout</span>`;
        btnAuth.classList.add('logged-in');
      } else {
        btnAuth.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> <span>Login to Sync</span>`;
